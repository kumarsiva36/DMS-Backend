        ...
        if(isset($request->staff_id) && $request->staff_id > 0){
            ...
            if(empty($isPermissionGiven)){
                ...
                foreach($involedOrders as $order){
                    ...
                    if(DashBoard::getOnGoingOrdersFunction($whereCondition,$request,true)!="")
    ...
    public static function getOnGoingOrdersFunction($whereCondition,$request,$condition){
        if($condition){
            if($request->workspaceType === "Buyer"){
                $orders = Order::where($whereCondition)->where('orders.status','1')
                        ...
                        ->select('orders.id','orders.style_no','order_factory.name as factoryName','order_pcu.name as pcuName','workspace.name as workspaceName')
                        ->first();
            }
                ...
                $orders = Order::where($whereCondition)->where('orders.status','1')
                            ...
                            ->leftjoin('workspace','workspace.id','orders.workspace_id')
                            ->select('orders.id','orders.style_no','order_pcu.name as pcuName','order_buyer.name as buyerName','workspace.name as workspaceName')
                            ->first();
            }
                ...
                $orders = Order::where($whereCondition)->where('orders.status','1')
                            ...
                            ->leftjoin('order_factory','order_factory.id','orders.factory_id')
                            ->leftjoin('workspace','workspace.id','orders.workspace_id')
                            ->select('orders.id','orders.style_no','order_buyer.name as buyerName','order_factory.name as factoryName','workspace.name as workspaceName')
                            ->first();
            }
        }else{
            if($request->workspaceType === "Buyer"){
