...
use App\Jobs\DelayOrderStatusDailyJob;
...
class DelayEmailScheduleSettings extends Controller
{
    /* The Delayed Order Email Daily */
    public static function delayOrderStatus(){
        //$isOrderStatusChecked = EmailScheduleSettings::where('email_schedule_task_id',2)->where('user_id',8)->get();
        $isOrderStatusChecked = EmailScheduleSettings::where('email_schedule_task_id',2)->get();
        foreach($isOrderStatusChecked as $order){
            if(str_contains($order->days,date('D'))){
                ...
                if($userType ==="Staff"){
                    $staffRoleHasPermission = Staff::select('role_id','company_id')->where('id',$user->id)->first();
                    ...
                    if(empty($isPermissionGiven)){
                        ...
                        foreach($involedOrders as $order){
                            ...
                            $allOrders = Order::where($whereCondition)
                                            ->first();
                ...
                foreach($orders as $order){
                    ...
                    $details['delivery_date'] = $delivery_date != null ? date($dateFormat,strtotime($delivery_date)):$delivery_date;
                    /* Task Details Start*/
                    $Data=[
                    ...
                    foreach ($taskDetails as $tasks){
                        ...
                        if($tasks->task_accomplished_date === null && $tasks->task_schedule_start_date != null
                        && $tasks->task_schedule_end_date != null && strtotime(date('Y-m-d'))>strtotime($tasks->task_schedule_end_date)){
                    ...
                    $cutArr['updatedQuantity']=DelayEmailScheduleSettings::getUpdatedProductionSum($order,"Cut");
                    $cutArr['lastupdatedDate']=DelayEmailScheduleSettings::getLastUpdatedProductionDate($order,"Cut");
                    $cutArr['pendingQuantity']=$cutArr['totalQuantity'] - $cutArr['updatedQuantity'];
                    $cutArr['noOfDays']=DelayEmailScheduleSettings::dateDifference($order->cutting_end_date);
                    $cutArr['status']=DelayEmailScheduleSettings::getTheStatus($order->cutting_start_date,$order->cutting_end_date,$cutArr['lastupdatedDate'],$cutArr['totalQuantity'],$cutArr['updatedQuantity']);
                    // Log::info($cutArr['noOfDays']."-----".$cutArr['status']);
                    if($cutArr['updatedQuantity'] >=0 && $cutArr['status']==="Delay"){
                    ...
                    $sewArr['updatedQuantity']=DelayEmailScheduleSettings::getUpdatedProductionSum($order,"Sew");
                    $sewArr['lastupdatedDate']=DelayEmailScheduleSettings::getLastUpdatedProductionDate($order,"Sew");
                    $sewArr['pendingQuantity']=$sewArr['totalQuantity'] - $sewArr['updatedQuantity'];
                    $sewArr['noOfDays']=DelayEmailScheduleSettings::dateDifference($order->sewing_end_date);
                    $sewArr['status']=DelayEmailScheduleSettings::getTheStatus($order->sewing_start_date,$order->sewing_end_date,$sewArr['lastupdatedDate'],$sewArr['totalQuantity'],$sewArr['updatedQuantity']);
                    // Log::info($sewArr['noOfDays']."-----".$sewArr['status']);
                    if($sewArr['updatedQuantity'] >=0 && $sewArr['status']==="Delay"){
                    ...
                    $packArr['updatedQuantity']=DelayEmailScheduleSettings::getUpdatedProductionSum($order,"Pack");
                    $packArr['lastupdatedDate']=DelayEmailScheduleSettings::getLastUpdatedProductionDate($order,"Pack");
                    $packArr['pendingQuantity']=$packArr['totalQuantity'] - $packArr['updatedQuantity'];
                    $packArr['noOfDays']=DelayEmailScheduleSettings::dateDifference($order->packing_end_date);
                    $packArr['status']=DelayEmailScheduleSettings::getTheStatus($order->packing_start_date,$order->packing_end_date,$packArr['lastupdatedDate'],$packArr['totalQuantity'],$packArr['updatedQuantity']);
                    // Log::info($packArr['noOfDays']."-----".$packArr['status']);
                    if($packArr['updatedQuantity'] >=0 && $packArr['status']==="Delay"){
                    ...
                    if(in_array("prodData",(array_keys($details))) || in_array("taskData",(array_keys($details)))){
                        ...
                        $i++;
                        //DelayOrderStatusDailyJob::dispatch($details);
                        if($is_consolidated_mail==0){
                            $details_arr['filename'] = "DelayOrderStatus_".date('Y_m_d')."_".$order->id."_".rand(100000,999999)."_".$user->id.".pdf";
                            $details_arr['pdf_path']= public_path() . '/Notifications/'.date('Y_m_d').'/'.$details_arr['filename'];
                            $pdf_generate = DelayEmailScheduleSettings::create_mail_pdf($details_arr);
                            if($pdf_generate==1){
                                try{
                                    DelayOrderStatusDailyJob::dispatch($details_arr);;
                                }catch(Exception $e){
                                    Log::info($e->getMessage(). ' TASK-->OrderDelay'." EMAIL-->".$user->email." NAME-->".$user->name." ID-->".$user->id);
                                }
                            }
                            $i=0;
                        }
    ...
    public static function create_mail_pdf($details){
        ...
        if(count($details)>0){
            ...
            $pdf = Pdf::loadView('PDF_DelayOrderStatus');
    ...
    public static function getTheStatus($startDate,$endDate,$lastUpdatedDate,$totalCount,$updatedCount){
        ...
        else if($endDate != null && $totalCount === $updatedCount && strtotime($lastUpdatedDate)>strtotime($endDate)){
            $status = "Delayed Completion";
        }
        else if($endDate != null && $totalCount !== $updatedCount && strtotime(date('Y-m-d'))>strtotime($endDate)){
            $status = "Delay";
        }
