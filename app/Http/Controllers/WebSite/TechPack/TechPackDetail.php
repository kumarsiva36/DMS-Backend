...
class TechPackDetail extends Controller
{
    /*Add Tech Pack */
    public function addTechPack(Request $request)
    {
        ...
        try {
            ...
            $teckPackArr['po_id'] = $request->po_id!='' ? $request->po_id : 0;
            $teckPackArr['parent_po_id'] = $request->parentpo_id!='' ? $request->parentpo_id : 0;
    ...
    public function viewTechPack(Request $request)
    {
        ...
        $whereCondition[] = ['techpack.status', '!=', '2'];
        if (isset($request->article_id) && intval($request->article_id > 0)) {
            $whereCondition[] = ['techpack.article_id', '=', $request->article_id];
        }
        if (isset($request->fabric_id) && intval($request->fabric_id > 0)) {
            $whereCondition[] = ['techpack.fabric_id', '=', $request->fabric_id];
        }
        if (isset($request->category_id) && intval($request->category_id > 0)) {
            $whereCondition[] = ['techpack.category_id', '=', $request->category_id];
        }
        if (isset($request->style_no) && ($request->style_no!='')) {
            $whereCondition[] = ['techpack.style_no', '=', $request->style_no];
        }
        if (isset($request->po_no) && ($request->po_no!='')) {
            $whereCondition[] = ['techpack.po_no', '=', $request->po_no];
        }
        if (isset($request->is_publish) && ($request->is_publish!='')) {
            $whereCondition[] = ['techpack.is_publish', '=', $request->is_publish];
        }
        if (isset($request->from_date) && isset($request->to_date) && $request->from_date != '' && $request->to_date == '') {
            ...
            $whereCondition[] = ['techpack.created_at', '>=', $from];
            $whereCondition[] = ['techpack.created_at', '<=', $to];
        }
        if (isset($request->from_date) && isset($request->to_date) && $request->from_date != '' && $request->to_date != '') {
            $from = date('Y-m-d 00:00:00', strtotime($request->from_date));
        ...
        $result = TechPack::getTechpackList($whereCondition,$request);

        //$tec = TechPack::where($whereCondition)->orderBy('id', 'DESC')->paginate(20, ['*'], 'page', $request->page);
        $res = json_encode(["status_code" => 200, "status" => "Success", "data" => $result, "filterData" => $this->techPackFilters($request)]);
        return CommonApp::webEncrypt($res);
    }
    /*Teckpack Filters */
    private function techPackFilters($request)
    {
        ...
        $filterArrayData = TechPack::select("article_id as id", "article_name as name", DB::raw("'article' as type"))
            ...
            ->union(
                TechPack::select("style_no as id", "style_no as name", DB::raw("'style' as type"))
                    ->where($whereCondition)
                    ->where("style_no", "!=", "")
                    ->groupBy("style_no")
            ...
            ->union(
                TechPack::select( "po_no as id", "po_no as name", DB::raw("'po' as type"))
                    ->where($whereCondition)
                    ->where("po_no", "!=", "")
                    ->groupBy("po_no")
                    ->orderBy("po_no", "asc")
        ...
        $filterArray['style'] = $filterArrayData->where('type', 'style')->values();
        $filterArray['po_no'] = $filterArrayData->where('type', 'po')->values();
        return $filterArray;
    }
    /*Edit Tech Pack */
        ...
        try {
            if($type == 'Garmentimage' || $type == 'MeasurementChart'){
                ...
                if($techpack_details_id!=0){
                    ...
                    ->orderBy("convert_images", 'asc')
                    ->orderBy("image_width", 'asc')->get();
                    //dd($referenceId,$techpack_details_id,$type);
                }else{
                    $files = TechPackImages::select("id as media_id", "techpack_type", "filepath", "orginalfilename", "reference_id", "convert_images","image_width","image_height","techpack_details_id")
                    ->where('reference_id', $referenceId)
                    ->where("techpack_type", $type)
            ...
            if (count($files) > 0) {
                foreach ($files as $key => $file) {
                    ...
                    $files[$key]->filepath = Storage::disk('s3')->temporaryUrl($file->filepath, '+45 minutes');
    ...
    public function deleteTechPackFile(Request $request)
    {
        ...
        $files = TechPackImages::select("id as media_id", "techpack_id", "techpack_type", "filepath", "orginalfilename", "reference_id", "convert_images","after_publish_edit","edit_reference_id")->where($whereCondition)->first();
        if (!empty($files)) {
            $whereConditionTechP = [
                ['workspace_id', '=', $request->workspace_id],
                ['company_id', '=', $request->company_id],
            ...
            $edit_referenceId = $request->edit_referenceId ?? 0;
            $is_publish = $teckP['is_publish'] ?? 0;
            $after_publish_edit = $files['after_publish_edit'];
            $edit_reference_id = $files['edit_reference_id'];
            $techpack_type = $files['techpack_type'];
            // if (!empty($teckP)) {
            //     if ($teckP['is_publish'] == "1" && $comments==0) {
            ...
            try {
                if($is_publish!='1' || $comments==1)
                {
                    ...
                    if($is_publish==1 && $comments==0 && $after_publish_edit==0){
                        $whereCondition1 = [
                            ['reference_id', '=', $request->reference_id],
                            ['id', '!=', $request->image_id],
                        ...
                        TechPackImages::where($whereCondition1)->update($DataArray);
                        TechPackImages::where('id', $request->image_id)->update(['all_reference_id' => $request->reference_id,'deleted'=>1,'after_publish_edit'=>1]);
                    }
                    elseif($is_publish==1 && $comments==0 && $after_publish_edit==1){
                        $whereCondition1 = [
                            ['edit_reference_id', '=', $edit_reference_id],
                            ['id', '!=', $request->image_id],
                            ['techpack_type', '=', $techpack_type],
                        ...
                        TechPackImages::where($whereCondition1)->update($DataArray);
                        TechPackImages::where('id', $request->image_id)->update(['all_reference_id' => DB::raw("CONCAT(all_reference_id, ',', '$edit_reference_id')"),'deleted'=>1,'after_publish_edit'=>1]);
                    }
                }
        ...
        $reference_id = $request->reference_id;
        if($is_publish==1 && $after_publish_edit==1 && $request->type!='Garmentimage' && $request->type!='MeasurementChart')
            $reference_id = $edit_referenceId;

        $getTeckPackImg = $this->getTechPackImages($reference_id, $request->type, '',$request->techpack_details_id ?? 0,$after_publish_edit);
    ...
    public function downloadTechPackPDF(Request $request)
    {
        ...
        //$updated_by = '-';
        if ($tec['updated_by'] > 0 && ($updated_by_all!='' || $updated_by_all!=" ")) {
                ...
                try {
                    ...
                    if (!empty($teckpackDetails)) {
                        ...
                        foreach ($teckpackDetails as $techpack) {
                            ...
                            $getT = TechPackDetails::where($whereCondition2)->select('techpack_type', 'techpack_details','edit_reference_id')->first();
                            if ($getT == null) {
                                $teckPackDetAr = [];
                                $teckPackDetAr['company_id'] = $request->company_id;
                                $teckPackDetAr['workspace_id'] = $request->workspace_id;
                                ...
                                $teckPackDetAr['created_by'] = $request->staff_id > 0 ? $request->staff_id : $request->user_id;
                                $teckPackDetAr['created_by_type'] = $request->staff_id > 0 ? "Staff" : "Admin";
                                $teckPackDetAr['is_publish'] = $teckP['is_publish'];
                                ...
                                $teckPackDetAr['updated_at'] = date('Y-m-d H:i:s');
                                if($getT->techpack_details != $techpack['techpack_detail'])
                                {
                                    $teckPackDetAr['updated_by'] = $request->staff_id > 0 ? $request->staff_id : $request->user_id;
                                    $teckPackDetAr['update_by_type'] = $request->staff_id > 0 ? "Staff" : "Admin";
                                }
                                $after_values[$techpack['techpack_type']] = $techpack['techpack_detail'];
                                $before_values[$getT->techpack_type] = $getT->techpack_details;
                                TechPackDetails::where($whereCondition2)->update($teckPackDetAr);
                                if($edit_referenceId!=0 && $is_publish==1){
                                    if($techpack['techpack_detail']!=$getT->techpack_details){
                                        $teckPackEditDetAr = [];
                                        $teckPackEditDetAr['company_id'] = $request->company_id;
                                        $teckPackEditDetAr['workspace_id'] = $request->workspace_id;
                                        $teckPackEditDetAr['user_id'] = $request->user_id;
                                        $teckPackEditDetAr['staff_id'] = $request->staff_id;
                                        $teckPackEditDetAr['techpack_id'] = $request->teckpack_id;
                                        $teckPackEditDetAr['techpack_type'] = $techpack['techpack_type'];
                                        $teckPackEditDetAr['techpack_details'] = $getT->techpack_details;
                                        $teckPackEditDetAr['reference_id'] = $request->edit_referenceId;
                                        $teckPackEditDetAr['created_at'] = date('Y-m-d H:i:s');
                                        TechPackEditDetails::insert($teckPackEditDetAr);
                                        $updateArr['after_publish_edit'] = 1;
                                        $updateArr['edit_reference_id'] = $request->edit_referenceId;
                                        TechPackDetails::where($whereCondition2)->update($updateArr);

                                        if($getT->edit_reference_id!=0){
                                            $whereCondition1 = [
                                                ['edit_reference_id', '=', $getT->edit_reference_id],
                                                ['techpack_type', '=', $techpack['techpack_type']],
                                                ['deleted', '=', 0],
                                                ['techpack_id', '=', $request->teckpack_id],
                                            ];
                                            $DataArray = array(
                                                "after_publish_edit" => 1,
                                                "edit_reference_id" =>$request->edit_referenceId,
                                                'all_reference_id' => DB::raw("CONCAT(all_reference_id, ',', '$getT->edit_reference_id')")
                                            );
                                            TechPackImages::where($whereCondition1)->update($DataArray);
                                        }else{
                                            $whereCondition2 = [
                                            ...
                                            TechPackImages::where($whereCondition2)->update($DataArray);
                                        }
                                    }else{
                                        $count = TechpackImages::where('techpack_id','=', $request->teckpack_id)->where('techpack_type','=', $techpack['techpack_type'])->where('edit_reference_id','=',$edit_referenceId)->count();
                                        if($count > 0){
                                            $teckPackEditDetAr = [];
                                            $teckPackEditDetAr['company_id'] = $request->company_id;
                                            $teckPackEditDetAr['workspace_id'] = $request->workspace_id;
                                            ...
                                            $teckPackEditDetAr['techpack_id'] = $request->teckpack_id;
                                            $teckPackEditDetAr['techpack_type'] = $techpack['techpack_type'];
                                            $teckPackEditDetAr['techpack_details'] = $techpack['techpack_detail'];
                                            $teckPackEditDetAr['reference_id'] = $request->edit_referenceId;
                                            $teckPackEditDetAr['created_at'] = date('Y-m-d H:i:s');
                                            TechPackEditDetails::insert($teckPackEditDetAr);
                                            $updateArr['after_publish_edit'] = 1;
    ...
    public function addNewTechPackComments(Request $request)
    {
        ...
        //$trans_arr['lang'] = $request->language=='en' ? 'jp' :'en';
        $trans_arr['translate'] = $is_translate;
        $translated_data = $request->techpack_details;
        if($is_translate){
            $trans_arr['lang']= 'jp';
            if(preg_match('/[\p{Hiragana}\p{Katakana}\p{Han}]/u', strip_tags($request->techpack_details))){
                $trans_arr['lang']= 'en';
            }
    ...
    /*Translate techpack comments */
    public function translateTechpackComment(Request $request){
        $request = CommonApp::webDecrypt($request->getContent());
        $validator = Validator::make((array)$request, [
            'id' => 'required',
            'techpack_details'=> 'required',
        ...
        $updateArr=[];
        $updateArr['is_translate'] = $is_translate;
        $updateArr['translated_data'] = $translated_data;
        TechPackComments::where('id','=',$request->id)->update($updateArr);
        $res = json_encode(["status_code" => 200, "status" => "Success", "message" => "Comment Translated Successfully"]);
        return CommonApp::webEncrypt($res);
    }
        ...
        if (!empty($tecDet)) {
            ...
            foreach ($tecDet as $techdata){
                if($i==0 || $j==0){
                    ...
                    $tp_type = $techdata['techpack_type'];
                }
                if($techdata['comment_type'] == 'Image' && $user_id==$techdata['user_id'] && $staff_id==$techdata['staff_id'] && $tp_type==$techdata['techpack_type']){
                    $id_arr[]=$techdata['id'];
                    $j++;
                }else{
                    if(count($id_arr)>1){
                ...
                $staff_id = $techdata['staff_id'];
                $i++;
                if(count($tecDet)==$i && $techdata['comment_type'] == 'Image'){
                    if(count($id_arr)>1){
                        $imgcount_arr=array_merge($id_arr,$imgcount_arr);
                        $last_id_arr[]=end($id_arr);
                    }
            ...
            foreach ($tecDet as $techdata) {
                ...
                $tecp['comment_data'] = $techdata['comment_type'] == 'Image' ? Storage::disk('s3')->temporaryUrl($techdata['comment_data'], '+45 minutes') : $techdata['comment_data'];
            ...
            $res = json_encode(["status_code" => 200, "status" => "Success", "data" => array("commentdetails" => $tecDetAry,"techpackdetails" => $techAry),'audio_upload'=>config('constant.techpack_comments_audio_enable'),'video_upload'=>config('constant.techpack_comments_video_enable')]);
    ...
    public function downloadNewTechPackPDF(Request $request)
    {
        ...
        if(isset($request->techpack_type) && $request->techpack_type!=''){
            ...
            $tecDet = TechPackComments::select("id", "techpack_type", "comment_type", "comment_data","user_id","staff_id","techpack_id","created_by_type","created_at","created_by","orginalfilename","convert_images","is_translate","translated_data")
    ...
    /* Upload Audio files for techpack comments */
    public static function addAudioFile(Request $request){
        ...
        if ($request->hasFile('audio')) {
            ...
            $filePath = $request->file('audio')->store('audio/'.$request->company_id.'/'.$request->workspace_id, 'public');
            try{
                ...
                if(count($result['prediction_raw']['transcription']) > 0){
                   ...
                   // $translated_data = $text_data;
                    if($is_translate){
                        $translated_data = Helper::translate($text_data,$trans_arr);
                    }
                }
    ...
    /* Upload Video files for techpack comments */
    public static function addTechpackCommentsVideoFile(Request $request){
        $header = $request;
        $validator = Validator::make($request->all(), [
            'company_id' => 'required',
            'workspace_id' => 'required',
        ...
        if ($request->hasFile('video')) {
            $filePath = $request->file('video')->store('video/'.$request->company_id.'/'.$request->workspace_id, 'public');
            try{
                ...
                $teckPackDetAr['workspace_id'] = $request->workspace_id;
                $teckPackDetAr['user_id'] = $request->user_id;
                $teckPackDetAr['comment_type'] = 'Video';
                $teckPackDetAr['comment_data'] = $text_data;
                $teckPackDetAr['techpack_type'] = $request->techpack_type;
                $teckPackDetAr['techpack_id'] = $request->techpack_id;
                $teckPackDetAr['staff_id'] = $request->staff_id;
        ...
        $res = json_encode(["status_code" => 401, "status" => "error", "message" => "Failed to upload the video file."]);
        return CommonApp::webEncrypt($res);
        return $res;
    }
