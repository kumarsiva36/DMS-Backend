...
namespace App\Http\Controllers\Mobile\Order\DataInput;
...
class GetCalendarData extends Controller
{
    /* Get the calendar data for Data Input */
    public  function getCalendarData(Request $request){
        ...
        $dataArr['knobChart']['pendingQuantity'] = $dataArr['knobChart']['totalQuantity']-$dataArr['knobChart']['completedQuantity'];
        // $todaysValues=0;
    	...
    	foreach ($prodDetails as $value) {
            ...
            // if($todaysValues === 0 && $value->holiday_flag != 1){
            //     $j++;
            // }
        ...
        $reqRate=round(($dataArr['knobChart']['totalQuantity']-$dataArr['knobChart']['completedQuantity'])/($j>0 ? $j : 1));
        ...
        // $dataArr['delivery_dates']=$delivey_dates;
        // $dataArr['last_delivery_date']=$last_delivey_date;
        ...
        $dataArr['delivery_date']=$delivery_date;
        ...
        $dataArr['delivery_date_exceed']=$delivery_date_exceed;

        $res = json_encode(["status_code"=>200,"status" =>"Success","ProductionType"=>$request->type_of_production,"data"=>$dataArr],200);
        return CommonApp::apiEncrypt($res);
    }
    /* Get Color Name */
    ...
    public static function getSkuUpdatedQuantity($whereCondition){
        $sumOfSku = UpdateSkuQuantity::where($whereCondition)->sum('updated_quantity');
        return $sumOfSku;
    }

    /* Get SKU For Given date */
    public static function getSkuForDate(Request $request){
        $request= CommonApp::apiDecrypt($request->getContent());
        $validator = Validator::make((array)$request, [
            'company_id' => 'required',
            'order_id' => 'required',
        ...
        $whereConditions = [
            ...
            ['order_id','=',$request->order_id],
            ['type_of_production','=',$request->type_of_production],
            ['sku_date','=',date('Y-m-d',strtotime($request->sku_date))]
        ];
        $whereConditions1 = [
            ['workspace_id','=',$request->workspace_id],
            ['company_id', '=', $request->company_id],
        ...
        $isAccomplished = "0";
        if(isset($request->type_of_production) && $request->type_of_production === "Cut" && $order->cutting_accomplished_date != null)
            $isAccomplished = "1";
        if(isset($request->type_of_production) && $request->type_of_production === "Sew" && $order->sewing_accomplished_date != null)
            $isAccomplished = "1";
        if(isset($request->type_of_production) && $request->type_of_production === "Pack" && $order->packing_accomplished_date != null)
            $isAccomplished = "1";
        $skuArr=[];
        if(count($dayByDaySKUupdates)>0){
            foreach($dayByDaySKUupdates as $dayUpdate){
                $array=[];
                $array['color_id']= $dayUpdate->color_id;
                $array['size_id']= $dayUpdate->size_id;
                $array['colorName']= GetCalendarData::getColorName($dayUpdate->color_id);
                $array['sizeName']= GetCalendarData::getSizeName($dayUpdate->size_id);
                $array['total_quantity']=(GetCalendarData::getSkuDetails($whereConditions1,$dayUpdate->color_id,$dayUpdate->size_id))->sku_quantity;
                $array['updated_quantity'] = (int)((GetCalendarData::getUpdatedQuantity($whereConditions2,$dayUpdate->color_id,$dayUpdate->size_id))->updated_quantity);
                $array['pending_quantity'] = $array['total_quantity']-$array['updated_quantity'];
                $array['today_updated_quantity'] = $dayUpdate->updated_quantity;
                $array['date_updated'] = $dayUpdate->sku_date;
                $skuArr[]=$array;
            }
        }
        else{
            $skuData = OrderSku::where($whereConditions1)->get();
            foreach($skuData as $dayUpdate){
                $array=[];
                $array['color_id']= $dayUpdate->sku_color_id;
                $array['size_id']= $dayUpdate->sku_size_id;
                $array['colorName']= GetCalendarData::getColorName($dayUpdate->sku_color_id);
                $array['sizeName']= GetCalendarData::getSizeName($dayUpdate->sku_size_id);
                $array['total_quantity']=(GetCalendarData::getSkuDetails($whereConditions1,$dayUpdate->sku_color_id,$dayUpdate->sku_size_id))->sku_quantity;
                $array['updated_quantity'] =(int)((GetCalendarData::getUpdatedQuantity($whereConditions2,$dayUpdate->sku_color_id,$dayUpdate->sku_size_id))->updated_quantity);
                $array['pending_quantity'] = $array['total_quantity']-$array['updated_quantity'];
                $array['today_updated_quantity'] = 0;
                $array['date_updated'] = "";
                $skuArr[]=$array;
            }
        }
        $res = json_encode(["status_code" => 200, "status"=>"Success","isAccomplished"=>$isAccomplished,"data"=>$skuArr]);
        return CommonApp::apiEncrypt($res);
    }

    /* Get The SKU Details */
    public static function getSkuDetails($whereCondition,$colorId,$sizeId){
        $whereCondition[]=["sku_color_id",'=',$colorId];
        $whereCondition[]=["sku_size_id",'=',$sizeId];
        $sku = OrderSku::where($whereCondition)->select("sku_quantity")->first();
        return $sku;
    }
    /* Get the Updated Quantity */
    public static function getUpdatedQuantity($whereCondition,$colorId,$sizeId){
        $whereCondition[]=["color_id",'=',$colorId];
        $whereCondition[]=["size_id",'=',$sizeId];
        $updates =UpdateSkuQuantity::where($whereCondition)
        ->select(DB::raw('SUM(updated_quantity) as updated_quantity'))->first();
        return $updates;
    }
}
