...
<body style="font-family: poppins,arialuni,notosansjp,poppins-semibold; font-size: 12px;width:100%">
    <div>
        ...
        <div style="font-size:18px;font-weight: 600;text-align:center; padding: 1px 3px 5px;">
            <span><strong>{{ trans('WebSite.TaskReport') }}</strong> - <strong>{{  trans('WebSite.'.$pendingTask['statusFilter']) }}
            </strong></span>
        </div>
            ...
            @if (!empty($tasks['taskData']))
                        ...
                        <table style="margin: 0;padding: 0; width:100%;" class="tableType" >
                            <tr style="background-color: #C4E1DD;">
                                ...
                                <td style="line-height: 1;">
                                    ...
                                    <p style="padding-left: 0px;color:#178677;word-wrap: break-word;margin-top:0px;margin-bottom:5px;"><strong>{{ $tasks['orderNo'] }}</strong></p>
                                </td>
                            </tr>
                ...
                <table style="margin-top : 5px;width: 100%; border-collapse: collapse" cellspacing="1px" class="mainTable">
                    <tr style="background-color: #f0efef;font-weight:600;font-family: poppins,arialuni,notosansjp;font-size:12px">
                        ...
                        @if ($pendingTask["statusFilter"]==="All" ||$pendingTask["statusFilter"]==="Completed" ||
                        $pendingTask["statusFilter"]==="DelCompletion" || $pendingTask["statusFilter"]==="Rescheduled" )
                            <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.AccomplishedDate') }}</strong></td>
                        @endif
                        ...
                        <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.Result') }}</strong></td>
                    ...
                    @foreach ( $tasks['taskData'] as $taskData )
                        <tr>
                            ...
                            @if ($pendingTask["statusFilter"]==="All" ||$pendingTask["statusFilter"]==="Completed" ||
                            $pendingTask["statusFilter"]==="DelCompletion" || $pendingTask["statusFilter"]==="Rescheduled" )
                                @if ($taskData['accomplishedDate'] == "")
                                    <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;">---</td>
                                @else
                                    <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;">{{ $taskData['accomplishedDate'] }}</td>
                                @endif
                            @endif
                            {{-- Person In Charge --}}
                            @if ($taskData['pic'] == "")
                            ...
                            @endif
                            {{-- Status --}}
                            @if ($taskData['accomplishedDate'] != "" && strtotime($taskData['accomplishedDate']) <= strtotime($taskData['scheduledDate']))
                            ...
                            @elseif ($taskData['accomplishedDate'] != "" && strtotime($taskData['accomplishedDate']) > strtotime($taskData['scheduledDate']))
                                <td style="text-align:center; background-color: #FFFFFF;">
                                    <img src="{{ public_path().'/images/SmileyGreen.svg' }}" width="{{config('constant.pdf_icon_width')}}px"/>
                                </td>
                        ...
                        @if (array_key_exists('subtasks',$taskData)>0)
                            @foreach ( $taskData['subtasks'] as $subtask )
                                <tr>
                                    ...
                                    @if ($pendingTask["statusFilter"]==="All" ||$pendingTask["statusFilter"]==="Completed" ||
                                    $pendingTask["statusFilter"]==="DelCompletion" || $pendingTask["statusFilter"]==="Rescheduled" )
                                        @if ($subtask['accomplishedDate'] == "")
                                            <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;">---</td>
                                        @else
                                            <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;">{{ $subtask['accomplishedDate'] }}</td>
                                        @endif
                                    @endif
                                    {{-- Person In Charge --}}
                                    @if ($subtask['pic'] == "")
                                    ...
                                    @endif
                                    {{-- Status --}}
                                    @if ($subtask['accomplishedDate'] != "" && strtotime($subtask['accomplishedDate']) <= strtotime($subtask['scheduledDate']))
                                        <td style="text-align:center; background-color: #FFFFFF;">
                                            <img src="{{ public_path().'/images/SmileyGreen.svg' }}" width="{{config('constant.pdf_icon_width')}}px"/>
                                        </td>
                                        <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.Completed') }}</strong></td>
                                    @elseif ($subtask['accomplishedDate'] != "" && strtotime($subtask['accomplishedDate']) > strtotime($subtask['scheduledDate']))
                                        <td style="text-align:center; background-color: #FFFFFF;">
                                            <img src="{{ public_path().'/images/SmileyGreen.svg' }}" width="{{config('constant.pdf_icon_width')}}px"/>
                                        </td>
                                        <td style="padding : 5px 5px 5px 10px; font-family: poppins,arialuni,notosansjp;"><strong>{{ trans('WebSite.DelayedComplete') }}</strong></td>
