...
<body style="font-family: poppins,arialuni,notosansjp,poppins-semibold; font-size: 12px;">
    <div>
        ...
        <div style="float:right; font-size:25px; font-weight:600; color: #8C878D; ">
            <strong>{{ trans('WebSite.dailyProdReport') }}</strong>
            <div
        ...
        <div style="font-size:18px;font-weight: 600;text-align:center; padding: 1px 3px 5px;">
            <span><strong>{{ trans('WebSite.dailyProdReport') }}</strong></span>
        </div>
        {{-- <div>
            ...
            <table style="width: 100%; border-collapse: collapse;font-size:10px;" cellspacing="1px" class="headTable">
                @if($dailyUpdates['useLogo']==1 && $dailyUpdates['userLogo']!='')
                    <tr style="">
                        ...
                        <td>
                            <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ $dailyUpdates['orderNo'] }}</strong>
                        </td>
                        <td style="width:10%;background-color: #f0efef;">
                            <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ trans('WebSite.Style') }}</strong>
                        </td>
                        <td>
                            <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ $dailyUpdates['styleNo'] }}</strong>
                        </td>
                        <td style="width:10%;background-color: #f0efef;">
                            <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ trans('WebSite.StartDate') }}</strong>
                        </td>
                        <td>
                ...
                <tr>
                    ...
                    <?php $td_i++;?>
                            ...
                            <td>
                                <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ $dailyUpdates["factory"] }}</strong>
                            </td>
                    @endif
                    @if (in_array("pcu",(array_keys($dailyUpdates))))
                    <?php $td_i++;?>
                            ...
                            <td>
                                <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ $dailyUpdates["pcu"] }}</strong>
                            </td>
                    @endif
                    @if (in_array("buyer",(array_keys($dailyUpdates))))
                    <?php $td_i++;?>
                            ...
                            <td>
                                <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ $dailyUpdates["buyer"] }}</strong>
                            </td>
                    @endif
                    @if($td_i<2)
                            <td colspan="2"></td>
                    @endif
                    <td style="width:10%;background-color: #f0efef;">
                        <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">{{ trans('WebSite.EndDate') }}</strong>
                    </td>
                    <td>
                        <strong style="font-family: poppins,arialuni,notosansjp,poppins-semibold;margin-left:5px;">
        ...
        <div style="margin: 2px 0; font-size:12px;">
            @if (count($dailyUpdates['advFilter'])>0)
                ...
                @endif
                @if (array_key_exists("type",$dailyUpdates['advFilter']))
                    <strong style="background-color: #E8E8E8;color:#606060;padding:1px 3px 3px;">
                        {{ trans('WebSite.'.$dailyUpdates['advFilter']['type']) }}</strong>
                @endif
                @if (array_key_exists("value",$dailyUpdates['advFilter']))
                    <strong style="background-color: #E8E8E8;color:#606060;padding:1px 3px 3px;">
                        {{ $dailyUpdates['advFilter']['operator'] }}</strong>
                @endif
                @if (array_key_exists("operator",$dailyUpdates['advFilter']))
                    <strong style="background-color: #E8E8E8;color:#606060;padding:1px 3px 3px;">
                        {{ $dailyUpdates['advFilter']['value'] }}</strong>
                @endif
    ...
    @foreach ($dailyUpdates['prodData'] as $data)
        ...
        if(isset($data['Cut_actual'])){
            ...
            if ($data['Cut_diff'] == 0)
                $diff='0 / 0';
            elseif ($data['Cut_diff'] < 0){
                $diff= '0 / '.abs($data['Cut_diff']) ;
                $cut_short_total += abs($data['Cut_diff']);
            }
            elseif ($data['Cut_diff'] > 0){
                $diff= $data['Cut_diff'].' / 0' ;
                $cut_excess_total += abs($data['Cut_diff']);
            }

            $cut_actual_total += $data['Cut_actual'];
            ...
            $cut_data.='<tr>
                            ...
                            <td style="padding : 5px 5px 10px 20px; font-family: poppins,arialuni,notosansjp;">'.$data['Cut_actual'].'</td>
        ...
        if(isset($data['Sew_actual'])){
            ...
            if ($data['Sew_diff'] == 0)
                $diff='0 / 0';
            elseif ($data['Sew_diff'] < 0){
                $diff= '0 / '.abs($data['Sew_diff']) ;
                $sew_short_total += abs($data['Sew_diff']);
            }
            elseif ($data['Sew_diff'] > 0){
                $diff= $data['Sew_diff'].' / 0' ;
                $sew_excess_total += abs($data['Sew_diff']);
            }

            $sew_actual_total += $data['Sew_actual'];
            ...
            $sew_data.='<tr>
                            ...
                            <td style="padding : 5px 5px 10px 20px; font-family: poppins,arialuni,notosansjp;">'.$data['Sew_actual'].'</td>
                            <td style="padding : 5px 5px 10px 20px; font-family: poppins,arialuni,notosansjp;">'.$diff.'</td>
                        </tr>';
        }
        if(isset($data['Pack_actual'])){
            ...
            if ($data['Pack_diff'] == 0)
                $diff='0 / 0';
            elseif ($data['Pack_diff'] < 0){
                $diff= '0 / '.abs($data['Pack_diff']);
                $pack_short_total += abs($data['Pack_diff']);
            }
            elseif ($data['Pack_diff'] > 0){
                $diff= $data['Pack_diff'].' / 0';
                $pack_excess_total += abs($data['Pack_diff']);
            }

            $pack_actual_total += $data['Pack_actual'];
            ...
            $pack_data.='<tr>
                            ...
                            <td style="padding : 5px 5px 10px 20px; font-family: poppins,arialuni,notosansjp;">'.$data['Pack_actual'].'</td>
                            <td style="padding : 5px 5px 10px 20px; font-family: poppins,arialuni,notosansjp;">'.$diff.'</td>
                        </tr>';
        }
        ?>
    ...
    @if ($cut_data!="")
        ...
        cellspacing="1px" class="mainTable">
            <tr style="background-color: #f0efef;font-weight:500; font-family: poppins,arialuni,notosansjp;font-size:14px;">
            ...
            {!! $cut_data !!}
            <tr>
                <td style="padding : 5px 5px 10px 20px; font-family: poppins,arialuni,notosansjp;font-size:14px;"><strong>{{ trans('WebSite.Total') }}</strong></td>
    ...
    <footer style="margin-top: 15px;width: 100%;background-color: #f0efef; font-size:12px;text-align:center;
            padding: 1px 3px 5px;">
            <strong>
                {!! trans('WebSite.PDFFooter',['date'=>date($dailyUpdates['dateFormat'],strtotime($dailyUpdates['endDate']))]) !!}
            </strong>
    </footer>
    <footer>
        <script type="text/php">
            if (isset($pdf)) {
               ...
               $pdf->page_text(28,815, "{{ trans('WebSite.dailyProdReport') }}", $font, 9, array(0, 0, 0));
