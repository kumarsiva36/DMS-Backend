...
<body style="font-family: poppins,arialuni,notosansjp,poppins-semibold;font-size: 14px;width:100%">
    ...
    <div style="background-color: #EBEBEB; color: #000000; font-weight:600;padding-bottom:5px;padding-top:0px;">
        <strong><span style="margin-left:10px;">{{ $chats['title']}}</span> <span>{{ " - ".$chats['subtitle'] }}</span></strong>
    </div>
    @foreach ( $chats['taskChats'] as $chat )
        @if (($chats['user_type'] === 'User' && $chat['staff_id']== "0" &&$chat['user_id'] == $chats['user_id'])
        || ($chats['user_type'] === 'Staff' && $chat['staff_id']> "0" &&$chat['staff_id'] == $chats['user_id']) )
            ...
            word-wrap: break-word;border: 1px solid #B2D9D6;border-radius:7px;">
                ...
                @if ($chat['text_type'] === "image")
                    <div style="margin-left:10px;text-align: left"><img style="margin-left:10px;" src={{ $chats['serverURL'].$chat['text'] }} height="150" width="150"/></div>
                @elseif ($chat['text_type'] === "pdf")
                    <div style="margin-left:10px; font:12px;text-align: left"><strong style="margin-left:10px;">{{ $chats['serverURL'].$chat['text'] }}</strong></div>
        ...
        @else
            ...
            word-wrap: break-word;border: 1px solid #E5E5E5;border-radius:7px;">
                ...
                @if ($chat['text_type'] === "image")
                    <div style="margin-left:10px;margin-right:10px;margin-left:15px;">
                        <img src={{ $chats['serverURL'].$chat['text'] }} height="150" width="150"/></div>
                @elseif ($chat['text_type'] === "pdf")
                    <div style="margin-left:10px; font:12px;margin-right:10px;margin-left:15px;">
                        <strong>{{ $chats['serverURL'].$chat['text'] }}</strong></div>
                @elseif ($chat['text_type'] === "excel")
                    <div style="margin-left:10px; font:12px;margin-right:10px;margin-left:15px;">
                        <strong>{{ $chats['serverURL'].$chat['text'] }}</strong></div>
                @elseif ($chat['text_type'] === "docx")
    ...
    @if (array_key_exists('subtasks',$chats) && count($chats['subtasks'])>0)
        @foreach ( $chats['subtasks'] as $subtaskChats )
            <div style="background-color: #EBEBEB; color: #000000; font-weight:600;padding-bottom:5px;padding-top:0px;margin-top:10px;">
                ...
                <span>{{ " - ".$subtaskChats['subtasktitle'] }}</span></strong>
            </div>
            @foreach ($subtaskChats['chats'] as $subtaskChat)
                @if (($chats['user_type'] === 'User' && $subtaskChat['staff_id']== "0" &&$subtaskChat['user_id'] == $chats['user_id'])
                || ($chats['user_type'] === 'Staff' && $subtaskChat['staff_id']> "0" &&$subtaskChat['staff_id'] == $chats['user_id']) )
                    <div style="float: right; width:45%;background-color:#B2D9D6;margin-top:5px;margin-bottom:3px;
                    word-wrap: break-word;border: 1px solid #B2D9D6;border-radius:7px;">
                        ...
                        @if ($subtaskChat['text_type'] === "image")
                            <div style="margin-left:10px;text-align: left"><img style="margin-left:10px;" src={{ $chats['serverURL'].$subtaskChat['text'] }} height="150" width="150"/></div>
                        @elseif ($subtaskChat['text_type'] === "pdf")
                            <div style="margin-left:10px; font:12px;text-align: left"><strong style="margin-left:10px;">{{ $chats['serverURL'].$subtaskChat['text'] }}</strong></div>
                ...
                @else
                    ...
                    word-wrap: break-word;border: 1px solid #E5E5E5;border-radius:7px;">
                        ...
                        @if ($subtaskChat['text_type'] === "image")
                            <div style="margin-left:10px;margin-right:10px;margin-left:15px;">
                                <img src={{ $chats['serverURL'].$subtaskChat['text'] }} height="150" width="150"/></div>
                        @elseif ($subtaskChat['text_type'] === "pdf")
                            <div style="margin-left:10px; font:12px;margin-right:10px;margin-left:15px;">
                                <strong>{{ $chats['serverURL'].$subtaskChat['text'] }}</strong></div>
                        @elseif ($subtaskChat['text_type'] === "excel")
                            <div style="margin-left:10px; font:12px;margin-right:10px;margin-left:15px;">
                                <strong>{{ $chats['serverURL'].$subtaskChat['text'] }}</strong></div>
                        @elseif ($subtaskChat['text_type'] === "docx")
