...
/*Trims Additional Info*/
$additional_info=[];
$trims_filter = 0;
if(!empty($data['trims_additional'])){
    foreach ($data['trims_additional'] as $additional) {
        $var = str_replace(' ','',$additional->label);

        if(in_array($var,$filter_type))
            $trims_filter = 1;

        $additional_info[$var."_arr"]= $additional_info[$var."_img"]= $additional_info[$var."_txt"]=[];
        $additional_info[$var."_i"]= $additional_info[$var."_count"]= $additional_info[$var."_j"]= $additional_info[$var."_status"]=0;
        $additional_info[$var."_user"]= $additional_info[$var."_date"]='';
    }
}
    ...
    if($arr['publish_status'] ==1)
    {
        ...
        if(!empty($data['trims_additional'])){
            foreach ($data['trims_additional'] as $additional) {
                ...
                if($arr['type']==$var){
                    $ref_id=$arr['reference_id'];
                    if($additional_info[$var."_i"]==0)
                        $refId = $arr['reference_id'];

                    if($refId==$arr['reference_id']){
                        if($arr['content_type']=='image'){
                            $additional_info[$var."_img"][]= $arr['content'];
                        }
                        if($arr['content_type']=='text'){
                            $additional_info[$var."_txt"][]= $arr['content'];
                        }
                        $additional_info[$var."_user"]   = $arr['user_type']=='user' ? $arr['username'] : $arr['staffname'];
                        $additional_info[$var."_date"]   = date('Y-m-d H:i:s',strtotime($arr['createdDate']));
                        $additional_info[$var."_status"] =$arr['status']??0;
                    }else{
                        $additional_info[$var."_arr"][$additional_info[$var."_j"]]['image'] = $additional_info[$var."_img"];
                        $additional_info[$var."_arr"][$additional_info[$var."_j"]]['text'] = $additional_info[$var."_txt"];
                        $additional_info[$var."_arr"][$additional_info[$var."_j"]]['refId'] =$ref_id;
                        $additional_info[$var."_arr"][$additional_info[$var."_j"]]['user'] = $additional_info[$var."_user"];
                        $additional_info[$var."_arr"][$additional_info[$var."_j"]]['date'] = $additional_info[$var."_date"];
                        $additional_info[$var."_arr"][$additional_info[$var."_j"]]['status'] = $additional_info[$var."_status"];

                        $additional_info[$var."_img"] = $additional_info[$var."_txt"] = [];
                        $refId = $arr['reference_id'];
                        $additional_info[$var."_user"] = $arr['user_type']=='user' ? $arr['username'] : $arr['staffname'];
                        $additional_info[$var."_date"] = date('Y-m-d H:i:s',strtotime($arr['createdDate']));
                        $additional_info[$var."_status"]=$arr['status']??0;
                        ...
                        $additional_info[$var."_j"]++;
                    }
                    $additional_info[$var."_i"]++;
                    $additional_info[$var."_count"]++;
                }
            }
        }
    }
...
if(!empty($data['trims_additional'])){
    foreach ($data['trims_additional'] as $additional) {
        $var = str_replace(' ','',$additional->label);
        if($additional_info[$var."_i"]==$additional_info[$var."_count"]){
            $additional_info[$var."_arr"][$additional_info[$var."_j"]]['image'] = $additional_info[$var."_img"];
            $additional_info[$var."_arr"][$additional_info[$var."_j"]]['text'] = $additional_info[$var."_txt"];
            $additional_info[$var."_arr"][$additional_info[$var."_j"]]['refId'] =$ref_id;
            $additional_info[$var."_arr"][$additional_info[$var."_j"]]['user'] = $additional_info[$var."_user"];
            $additional_info[$var."_arr"][$additional_info[$var."_j"]]['date'] = $additional_info[$var."_date"];
            $additional_info[$var."_arr"][$additional_info[$var."_j"]]['status'] = $additional_info[$var."_status"];
            $additional_info[$var."_j"] = $additional_info[$var."_i"]=0;
        }
    }
}
...
<body style="font-family: poppins,arialuni; font-size: 14px;">
    ...
    <table width="100%" style="border-collapse: collapse;font-family: poppins,arialuni;" cellspacing="1px" class="mainTable">
        @if (count($print_arr)>0 && (in_array('All',$filter_type) || in_array('printArtwork',$filter_type)))
            ...
            @php
                ...
                $border_bottom='';
            @endphp
            <tbody style="page-break-inside: avoid; page-break-after:always">
                ...
                <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                    ...
                    <td width="15%">
                        @foreach ($arr['image'] as $response)
                            <img src="{{ $response }}" title="PrintImage" width="100px"><br/>
                        @endforeach
                    </td>
                ...
                {{-- @foreach ($arr['image'] as $response)
                <tr>
                    <td colspan="3"><img src="{{ $response }}" title="PrintImage" style="max-width: 100%; max-height: 95%"></td>
                </tr>
                @endforeach --}}
        ...
        (in_array('All',$filter_type) || in_array('mainLabel',$filter_type) || in_array('washCare',$filter_type) || in_array('hangtag',$filter_type) || in_array('barcode',$filter_type) || $trims_filter==1))
            <tr style="background-color: #f0efef; color: #000000; font-weight:800; font-size:15px;">
            ...
            <tbody style="page-break-inside: avoid; page-break-after:always">
                @if(in_array('All',$filter_type) || in_array('mainLabel',$filter_type))
                    @foreach ( $mainlb_arr as $arr)
                        <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td width="15%">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $response }}" title="MainLabel" width="100px"><br/>
                                @endforeach
                            </td>
                            <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                @foreach ($arr['text'] as $response)
                        ...
                        {{-- @foreach ($arr['image'] as $response)
                        <tr>
                            <td colspan="3"><img src="{{ $response }}" title="MainLabel" style="max-width: 100%; max-height: 95%"></td>
                        </tr>
                        @endforeach --}}
                        @php
                            $title='';
                ...
                @if(in_array('All',$filter_type) || in_array('washCare',$filter_type))
                    @foreach ( $washlb_arr as $arr)
                        <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td width="15%">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $response }}" title="WashcareLabel" width="100px"><br/>
                                @endforeach
                            </td>
                            <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                @foreach ($arr['text'] as $response)
                        ...
                        {{-- @foreach ($arr['image'] as $response)
                        <tr>
                            <td colspan="3"><img src="{{ $response }}" title="WashcareLabel" style="max-width: 100%; max-height: 95%"></td>
                        </tr>
                        @endforeach --}}
                        @if ($arr['status']=='2')
                        {{-- <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td colspan="2">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $data['serverURL'].$response }}" title="MainLabel" style="max-width: 100%; max-height: 95%"><br/>
                                @endforeach
                            </td>
                        </tr> --}}
                        @endif
                ...
                @if(in_array('All',$filter_type) || in_array('hangtag',$filter_type))
                    @foreach ( $hanglb_arr as $arr)
                        <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td width="15%">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $response }}" title="HangTag" width="100px"><br/>
                                @endforeach
                            </td>
                            <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                @foreach ($arr['text'] as $response)
                        ...
                        {{-- @foreach ($arr['image'] as $response)
                        <tr>
                            <td colspan="3"><img src="{{ $response }}" title="HangTag" style="max-width: 100%; max-height: 95%"></td>
                        </tr>
                        @endforeach --}}
                    @endforeach
                @endif
                @if(in_array('All',$filter_type) || in_array('barcode',$filter_type))
                    @foreach ( $barlb_arr as $arr)
                        <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td width="15%">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $response }}" title="BarCode" width="100px"><br/>
                                @endforeach
                            </td>
                            <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                @foreach ($arr['text'] as $response)
                        ...
                        {{-- @foreach ($arr['image'] as $response)
                        <tr>
                            <td colspan="3"><img src="{{ $response }}" title="BarCode" style="max-width: 100%; max-height: 95%"></td>
                        </tr>
                        @endforeach --}}
                    @endforeach
                @endif
                ...
                {{-- Trims additional Info --}}
                @if(in_array('All',$filter_type) || $trims_filter==1 )
                    @foreach ($data['trims_additional'] as $additional)
                        @php
                            $var = str_replace(' ','',$additional->label);
                        @endphp
                        @if(in_array('All',$filter_type) || in_array($var,$filter_type))
                            @foreach ( $additional_info[$var."_arr"] as $arr)
                            <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                                <td style="padding : 5px 5px 10px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="15%" ><strong>{{ $additional->label }}</strong></td>
                                <td width="15%">
                                    @foreach ($arr['image'] as $response)
                                        <img src="{{ $response }}" title="{{ $additional->label }}" width="100px"><br/>
                                    @endforeach
                                </td>
                                <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                    @foreach ($arr['text'] as $response)
                ...
                @if(in_array('All',$filter_type) || in_array('polybag',$filter_type))
                    @foreach ( $poly_arr as $arr)
                        <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td width="15%">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $response }}" title="Polybag" width="100px"><br/>
                                @endforeach
                            </td>
                            <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                @foreach ($arr['text'] as $response)
                        ...
                        {{-- @foreach ($arr['image'] as $response)
                        <tr>
                            <td colspan="3"><img src="{{ $response }}" title="Polybag" style="max-width: 100%; max-height: 95%"></td>
                        </tr>
                        @endforeach --}}
                    @endforeach
                @endif
                @if(in_array('All',$filter_type) || in_array('carton',$filter_type))
                    @foreach ( $carton_arr as $arr)
                        <tr style="background-color: #ffffff; color: #000000; font-weight:600;">
                            ...
                            <td width="15%">
                                @foreach ($arr['image'] as $response)
                                    <img src="{{ $response }}" title="CartonBox" width="100px"><br/>
                                @endforeach
                            </td>
                            <td style="padding : 5px 5px 1px 10px; font-family: poppins,arialuni,notosansjp,poppins-semibold;" width="70%"><strong>
                                @foreach ($arr['text'] as $response)
                        ...
                        {{-- @foreach ($arr['image'] as $response)
                        <tr>
                            <td colspan="3"><img src="{{ $response }}" title="CartonBox" style="max-width: 100%; max-height: 95%"></td>
                        </tr>
                        @endforeach --}}
                    @endforeach
                @endif
    ...
    <div style="style:max-width:100%">
        @if(in_array('All',$filter_type) || in_array('printArtwork',$filter_type))
            @php
                $img_title=0;
            @endphp
            @foreach ( $print_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.print_image') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="PrintImage" style="max-width: 100%; max-height: 90%;" ><br>
                @endforeach
            @endforeach
        @endif
        ...
        @if(in_array('All',$filter_type) || in_array('mainLabel',$filter_type))
            ...
            @foreach ( $mainlb_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.main_lable_info') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="MainLabel" style="max-width: 100%; max-height: 90%"><br/>
                @endforeach
            @endforeach
        @endif
        ...
        @if(in_array('All',$filter_type) || in_array('washCare',$filter_type))
            ...
            @foreach ( $washlb_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.washcare_lable_info') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="WashcareLabel" style="max-width: 100%; max-height: 90%"><br/>
                @endforeach
            @endforeach
        @endif
        ...
        @if(in_array('All',$filter_type) || in_array('hangtag',$filter_type))
            ...
            @foreach ( $hanglb_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.hangtag_info') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="HangTag" style="max-width: 100%; max-height: 90%"><br/>
                @endforeach
            @endforeach
        @endif
        ...
        @if(in_array('All',$filter_type) || in_array('barcode',$filter_type))
            ...
            @foreach ( $barlb_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.barcode_stickers_info') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="Barcode" style="max-width: 100%; max-height: 90%"><br/>
                @endforeach
            @endforeach
        @endif
        ...
        @if(in_array('All',$filter_type) || in_array('polybag',$filter_type))
            ...
            @foreach ( $poly_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.polybag_sample') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="PolyBag" style="max-width: 100%; max-height: 90%"><br/>
                @endforeach
            @endforeach
        @endif
        ...
        @if(in_array('All',$filter_type) || in_array('carton',$filter_type))
            ...
            @foreach ( $carton_arr as $arr)
                @foreach ($arr['image'] as $response)
                    @if($img_title==0)
                        <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ trans('WebSite.carton_details') }}</p>
                    @endif
                    @php $img_title++; @endphp

                    <img src="{{ $response }}" title="Carton Box" style="max-width: 100%; max-height: 90%"><br/>
                @endforeach
            @endforeach
        @endif
                ...
                @foreach ( $data['trims_additional'] as $additional)
                    ...
                    @if(in_array('All',$filter_type) || in_array($var,$filter_type))
                        @foreach ( $additional_info[$var."_arr"] as $arr)
                            @foreach ($arr['image'] as $response)
                                @if($img_title==0)
                                    <p style="font-size: 18px; text-align:center; padding:0; margin:0;page-break-before: always">{{ $additional->label }}</p>
                                @endif
                                @php $img_title++; @endphp
                                    <img src="{{ $response }}" title="Image" style="max-width: 100%; max-height: 90%;"><br>
                            @endforeach
