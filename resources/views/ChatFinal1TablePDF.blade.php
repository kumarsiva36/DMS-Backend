    ...
    <?php
        function getNonTextType($type,$time=""){
            ...
            elseif($type == 3){
                $nonTextType=
                '<span style="font-size:8px;">'.$time.'</span>';
                /* '<span style="border-radius:50%;background-color:#009688;width:6px;height:6px;display: inline-block; "> </span>
                <span style="font-size:10px;color:#009688;margin-top:3px;"></span> */
            }
            elseif($type == 4){
                $nonTextType=/* '<span style="border-radius:50%;background-color:#529ED1;width:6px;height:6px;display: inline-block; "> </span>
                <span style="font-size:10px;color:#529ED1;margin-top:3px;"></span> */
                '<span style="font-size:8px;">'.$time.'</span>';
            }
            return $nonTextType;
        }
        ...
        @foreach ($chats['taskChats'] as $chat)
            @if ($chat['comment_type']==1)
                <tr class="{{ $chat['comment_type']===1 ? 'request' : 'response' }}">
                        ...
                        @if ($chat['text_type'] != "text")
                            @if ($chat['text_type'] === "image")
                                <div style="margin-left:10px;text-align: left;margin-top:5px">
                                    <img style="margin-left:10px;display: inline-block;margin-top:35px;" src={{ $chats['serverURL'].$chat['text'] }} height="100" />
                    ...
                    <td style="width:65%;padding-left:5px;padding-right:5px;">
                        @if ($chat['text_type'] === "text")
            ...
            @if (count($chats['taskChatsReplies'])>0)
                {{-- @foreach ($chats['taskChatsReplies'] as $replies) --}}
                    {{-- @if ($replies['comment_type']==2 && $replies['reply_to_id'] == $chat['id']) --}}
                        <tr class="response">
                            {{-- Check Chat Text Type Starts --}}
                            <td style="width:35%;padding-left:5px;padding-right:5px;">
                                {{-- @if ($replies['text_type'] != "text") --}}
                                    {{-- @if ($replies['text_type'] === "image")
                                        <div style="margin-left:10px;text-align: left;margin-top:5px">
                                            <img style="margin-left:10px;display: inline-block;margin-top:35px;"
                                            src={{ $chats['serverURL'].$replies['text'] }} height="100" />
                                        ...
                                        {{-- @foreach ($chats['taskChatsReplies'] as $replies)
                                            @if ($replies['comment_type']==2)
                                                @if ($replies['reply_to_id'] == $chat['id'] && $replies['text_type'] != 'text')
                                                    @if ($replies['text_type'] === "image")
                                                        <div style="text-align: left;margin-top:5px">
                                                            <img style="margin-left:10px;margin-top:35px;" src={{ $chats['serverURL'].$replies['text'] }}
                                                            height="100" />
                                                            {!!  getNonTextType($replies['comment_status'])  !!}
                                                        </div>
                                                    @else
                                                        <div style="font-size:12px;text-align: left;">
                                                            <strong style="margin-left:10px;cursor: pointer;text-decoration: underline;
                                                            word-break: break-all"><a href={{ $chats['serverURL'].$replies['text'] }}>
                                                            {{ $replies['original_name'] }}</a></strong>
                                                            {!!  getNonTextType($replies['comment_status'])  !!}
                                                        </div>
                                                    @endif
